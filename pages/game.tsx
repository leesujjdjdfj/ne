...
class SudokuGenerator {
  ...
  static generatePuzzle(difficulty: string): { puzzle: number[][]; solution: number[][] } {
...
const BaseballSinglePlayer: FC<{ nickname: string; gameDigits: number }> = ({ nickname, gameDigits }) => {
  ...
  const [guesses, setGuesses] = useState<{ guess: string; result: string; isSuccess: boolean }[]>([]);
  const [guessInput, setGuessInput] = useState('');
  const [remainingTurns, setRemainingTurns] = useState(maxTurns);
  const [myScore, setMyScore] = useState(0);
  const [systemMessages, setSystemMessages] = useState<string[]>([]);
  const [isGameEnded, setGameEnded] = useState(false);
  const [gameOverInfo, setGameOverInfo] = useState<{ isWin: boolean; message: string } | null>(null);
  ...
  const addSystemMessage = (message: string) => setSystemMessages(prev => [...prev.slice(-10), message]);
...
// 스도쿠 플레이어 컴포넌트
// ==================================================================
const SudokuPlayer: FC<{ nickname: string; difficulty: string; mode: string; roomCode?: string }> = ({ nickname, difficulty, mode, roomCode }) => {
    ...
    return (
        <div className="sudoku-ui">
            ...
            <header className="sudoku-header">
                <div className="sudoku-header-top"><h1>Sudoku</h1><button onClick={() => router.push('/sudoku-setup')} className="new-game-btn">새 게임</button></div>
                <div className="game-info">
                    <div className="info-item">난이도: {difficulty}</div>
                    <div className="info-item">실수: {mistakes} / 3</div>
                    {mode === 'multi' && <div className="info-item">방 코드: {roomCode}</div>}
                </div>
            </header>
            ...
            <footer className="sudoku-footer">
                <div id="numpad" className="numpad">
                    {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(num => <button key={num} onClick={() => handleNumpadClick(num)}>{num}</button>)}
...
const GamePage: FC = () => {
  ...
  if (type === 'sudoku') {
    return <SudokuPlayer nickname={nickname} difficulty={difficulty as string || 'easy'} mode={mode as string} roomCode={roomCode as string} />;
  }
